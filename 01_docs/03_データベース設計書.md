...
erDiagram
    ...
    users ||--o{ pay_it_forward_payments : "makes"
    tasks ||--o{ task_progress : "tracks"
    tasks ||--o{ reminders : "has"
    pay_it_forward_payments ||--o{ pay_it_forward_messages : "includes"
    ...
    pay_it_forward_messages {
        uuid id PK
        uuid payment_id FK
        text message
        boolean is_displayed
        uuid displayed_to_user_id FK
        timestamp displayed_at
        timestamp created_at
    }

    pay_it_forward_stats {
        uuid id PK
        integer total_payments_count
        bigint total_amount
        integer new_users_count
...
**備考**: 現在のビジネスモデル（Pay It Forward）では基本的に使用しない。将来的な機能拡張（プレミアムプラン等）のために残置。
...
### 3.10 pay_it_forward_payments（恩送り支払い）

**概要**: 恩送り（Pay It Forward）の支払い記録を管理

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---------|-----|------|-----------|------|
| id | UUID | NOT NULL | uuid_generate_v4() | 主キー |
| user_id | UUID | NOT NULL | - | ユーザーID（外部キー） |
| payment_type | VARCHAR(50) | NOT NULL | - | 支払いタイプ（high_priority_completion, all_tasks_completion） |
| amount | INTEGER | NOT NULL | - | 支払い金額（円） |
| stripe_payment_id | VARCHAR(255) | NULL | - | Stripe決済ID |
| status | VARCHAR(50) | NOT NULL | 'pending' | ステータス（pending, completed, failed, refunded） |
| created_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 作成日時 |
...
**インデックス**:
- `idx_pay_it_forward_payments_user_id` ON user_id
- `idx_pay_it_forward_payments_status` ON status
- `idx_pay_it_forward_payments_created_at` ON created_at

**payment_type の種類**:
- `high_priority_completion`: 優先度高タスク完了時の恩送り
- `all_tasks_completion`: 全タスク完了時の感謝の支払い

---

### 3.11 pay_it_forward_messages（恩送りメッセージ）

**概要**: 恩送り時にユーザーが残したメッセージを管理

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---------|-----|------|-----------|------|
| id | UUID | NOT NULL | uuid_generate_v4() | 主キー |
| payment_id | UUID | NOT NULL | - | 支払いID（外部キー） |
| message | TEXT | NOT NULL | - | メッセージ内容（最大200文字） |
| is_displayed | BOOLEAN | NOT NULL | false | 表示済みフラグ |
| displayed_to_user_id | UUID | NULL | - | 表示先ユーザーID（外部キー） |
| displayed_at | TIMESTAMP | NULL | - | 表示日時 |
| created_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 作成日時 |

**制約**:
- PRIMARY KEY: id
- FOREIGN KEY: payment_id REFERENCES pay_it_forward_payments(id) ON DELETE CASCADE
- FOREIGN KEY: displayed_to_user_id REFERENCES users(id) ON DELETE SET NULL
- CHECK: LENGTH(message) <= 200

**インデックス**:
- `idx_pay_it_forward_messages_payment_id` ON payment_id
- `idx_pay_it_forward_messages_displayed` ON is_displayed WHERE is_displayed = false
- `idx_pay_it_forward_messages_created_at` ON created_at

**備考**:
- 未表示のメッセージ（is_displayed = false）をランダムに選んで新規ユーザーに表示
- 表示したらis_displayedをtrueに更新し、displayed_to_user_idとdisplayed_atを記録

---

### 3.12 pay_it_forward_stats（恩送り統計）

**概要**: 恩送りの統計情報を管理（単一レコード）

| カラム名 | 型 | NULL | デフォルト | 説明 |
|---------|-----|------|-----------|------|
| id | UUID | NOT NULL | uuid_generate_v4() | 主キー |
| total_payments_count | INTEGER | NOT NULL | 0 | 累計恩送り人数 |
| total_amount | BIGINT | NOT NULL | 0 | 累計恩送り金額（円） |
| new_users_count | INTEGER | NOT NULL | 0 | 新規ユーザー数 |
| last_updated_at | TIMESTAMP | NOT NULL | CURRENT_TIMESTAMP | 最終更新日時 |
...
**備考**:
- テーブルには常に1レコードのみ存在
- 恩送りがあるたびに `total_payments_count` と `total_amount` をインクリメント
- 新規ユーザー登録時に `new_users_count` をインクリメント
- 条件分岐メッセージの判定: `total_payments_count > new_users_count`

**初期データ挿入SQL**:
```sql
INSERT INTO pay_it_forward_stats (id, total_payments_count, total_amount, new_users_count)
...
### 5.11 pay_it_forward_payments テーブル
...
CREATE INDEX idx_pay_it_forward_payments_user_id ON pay_it_forward_payments(user_id);
CREATE INDEX idx_pay_it_forward_payments_status ON pay_it_forward_payments(status);
CREATE INDEX idx_pay_it_forward_payments_created_at ON pay_it_forward_payments(created_at);

-- payment_type の値検証
ALTER TABLE pay_it_forward_payments
ADD CONSTRAINT check_payment_type
CHECK (payment_type IN ('high_priority_completion', 'all_tasks_completion'));
...
### 5.12 pay_it_forward_messages テーブル

```sql
CREATE TABLE pay_it_forward_messages (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    payment_id UUID NOT NULL REFERENCES pay_it_forward_payments(id) ON DELETE CASCADE,
    message TEXT NOT NULL CHECK (LENGTH(message) <= 200 AND LENGTH(message) > 0),
    is_displayed BOOLEAN NOT NULL DEFAULT false,
    displayed_to_user_id UUID REFERENCES users(id) ON DELETE SET NULL,
    displayed_at TIMESTAMP,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

CREATE INDEX idx_pay_it_forward_messages_payment_id ON pay_it_forward_messages(payment_id);
CREATE INDEX idx_pay_it_forward_messages_displayed ON pay_it_forward_messages(is_displayed)
    WHERE is_displayed = false;
CREATE INDEX idx_pay_it_forward_messages_created_at ON pay_it_forward_messages(created_at);
```

### 5.13 pay_it_forward_stats テーブル

```sql
CREATE TABLE pay_it_forward_stats (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    total_payments_count INTEGER NOT NULL DEFAULT 0 CHECK (total_payments_count >= 0),
    total_amount BIGINT NOT NULL DEFAULT 0 CHECK (total_amount >= 0),
    new_users_count INTEGER NOT NULL DEFAULT 0 CHECK (new_users_count >= 0),
    last_updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP
);

-- 初期データ挿入（単一レコードのみ）
INSERT INTO pay_it_forward_stats (total_payments_count, total_amount, new_users_count)
VALUES (0, 0, 0);

-- 統計更新用の関数
CREATE OR REPLACE FUNCTION increment_payment_stats(p_amount INTEGER)
RETURNS VOID AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION increment_new_user_stats()
RETURNS VOID AS $$
BEGIN
    UPDATE pay_it_forward_stats
    SET new_users_count = new_users_count + 1,
        last_updated_at = CURRENT_TIMESTAMP;
END;
$$ LANGUAGE plpgsql;
```
...
|---------|------|---------|--------|
| 1.0 | 2025-10-05 | 初版作成 | - |
| 1.1 | 2025-11-09 | 恩送り機能用テーブル追加（pay_it_forward_payments, pay_it_forward_messages, pay_it_forward_stats） | - |
